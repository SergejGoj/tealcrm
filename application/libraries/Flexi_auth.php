...
include_once('Flexi_auth_lite.php');
...
class Flexi_auth extends Flexi_auth_lite
{
	public function __construct()
	{
		...
		// load Teal Global Vars
		$this->CI->load->model('teal_global_vars');
	}
	...
	public function login($identity = FALSE, $password = FALSE, $remember_user = FALSE)
	{
		if ($this->CI->flexi_auth_model->login($identity, $password, $remember_user))
		{
			...
			// upon successful login we set the global session variables
			$this->CI->teal_global_vars->set_all_global_vars();
	...
	public function resend_activation_token($identity)
	{
		...
		// If company is already activated.
		if ($active_status == 1)
		{
		...
		else if ($this->CI->flexi_auth_model->deactivate_user($user_id))
		{
			...
			// Set email data.
			$email_to = $email;
			$email_title = ' - Company Activation';
	 ...
	 * Sends the user a verification email to their new email address, the user must then click a link within the email to update their companies email address.
	 * This will help prevent a user accidentally locking themselves out of their company if they forget their password, as they can request a new password
	...
	public function insert_user($email, $username = FALSE, $password, $user_data, $group_id = FALSE, $activate = FALSE)
	{
		...
		if ($user_id)
		{
			...
			if ($activate)
			{
				// If an company activation time limit is set by the config file, retain activation token.
			...
			// Prepare company activation email.
			// If the $activation_token is not empty, the company must be activated via email before the user can login.
	...
	/**
	 * update_user
	 * Updates the main user company table and any linked custom user tables with the submitted data.
	 *
	 * @return void
	 ...
	 * Delete users that have not activated their company within a set time period.
	 *
	 * @return bool
	 * @author Rob Hussey
	 */
	 ...
	 * Get users that have not activated their company within a set time period.
	 *
	 * @return bool
	 * @author Rob Hussey
	 */
