...
class Notes extends App_Controller {
   ...
   /**
	...
	public function index(){
	    ...
	    // select
	    $notes->select('*, (SELECT company_name FROM sc_companies WHERE company_id = sc_notes.company_id) as company_name, (SELECT CONCAT((first_name),(" "),(last_name)) AS name FROM sc_people WHERE people_id = sc_notes.people_id) AS people_name');
		...
		if(!empty($_SESSION['search']['notes'])){
			...
			foreach($_SESSION['search']['notes'] as $key => $value){
				if($key != "search_type" && $key != "date_entered_start" && $key != "date_entered_end" && $key != "date_modified_start" && $key != "date_modified_end" && $key != "company_viewer" && $key != "person_viewer" && $key != "deal_value_start" && $key != "deal_value_end" && $key != "expected_close_date_start" && $key != "expected_close_date_end" && $key != "filename_original" && $key != "assigned_user_id"){
					$notes->like($key, $value);
				}
				
				if($key == "assigned_user_id")
				{
					$notes->where_in($key , $value);
				}
				...
				if($key == "filename_original")
				{
				if($value=='Yes')
				{
				$notes->where('CHAR_LENGTH(filename_original) >', 1);
				}
				else if($value=='No')
				{
				$notes->where('filename_original', '');
				}
				}
