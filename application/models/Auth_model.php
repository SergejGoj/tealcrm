...
class Auth_model extends CI_Model {
	 ...
	 * Then if defined via the '$instant_activate' var, automatically log the user into their company.
	 */
	function register_account()
	{
		...
		if ($this->form_validation->run())
		{
			...
			// Set whether to instantly activate company.
			...
			// #2. Set whether to automatically activate the company upon registration, default is FALSE.
			// Note: An company activation email will be automatically sent if auto activate is FALSE, or if an activation time limit is set by the config file.
			...
			if ($response)
			{
				...
				// Bear in mind, if registration has been set to require the user activates their company, they will already be receiving an activation email.
				...
				// This is an example of how to log the user into their company immeadiately after registering.
				// This example would only be used if users do not have to authenticate their company via email upon registration.
	 ...
	 * Resends a new company activation token to a users email address.
	 */
	...
	/**
	 * update_account
	 * Updates a users company and profile data.
	 */
	function update_account()
	{
		...
		if ($this->form_validation->run())
		{
			...
			// Get user profile data from input.
			// IMPORTANT NOTE: As we are updating multiple tables (The main user company and user profile tables), it is very important to pass the
	 ...
	 * The first option simply allows the users to change their email address along with the rest of their company data via entering it into a form fields.
	 ...
	 * Note: The address book table ('demo_user_address') is used in this demo as an example of relating additional user data to the auth libraries company tables.
	 */
