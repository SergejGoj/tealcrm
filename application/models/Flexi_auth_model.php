...
class Flexi_auth_model extends Flexi_auth_lite_model
{
	...
	/**
	 * insert_user
	 * Inserts user company and profile data, returning the users new id.
	 *
	 * @return bool
	 * @author Rob Hussey
	 */
	public function insert_user($email, $username, $password, $custom_data = FALSE, $group_id = FALSE)
	{
	    ...
	    // Create main user company.
		$this->db->insert($this->auth->tbl_user_account, $sql_insert);

		###+++++++++++++++++++++++++++++++++###

		// Custom user data table(s).
		// Get newly created User Company id for join with custom user data table(s).
	...
	/**
	 * update_user
	 * Updates the main user company table and any linked custom user tables.
	 *
	 * @return bool
	 * @author Rob Hussey
	...
	public function update_user($user_id, $user_data)
	{
		...
		// Add any additional custom data columns from the main user company table to the array.
		foreach($this->auth->database_config['user_acc']['custom_columns'] as $column)
		{
	...
	/**
	 * delete_user
	 * Deletes a user company and all linked custom user tables from the database.
	 *
	 * @return bool
	 * @author Rob Hussey
	 * @author Phil Sturgeon
	 ...
	 * Example: If user #1's EMAIL is 'x@y.com' and user #2's USERNAME is 'x@y.com', neither would be able to login to their company.
	 *
	 * @return bool
	 * @author Rob Hussey
	 */
	...
	/**
	 * activate_user
	 * Activates a users company allowing them to login to their company.
	...
	/**
	 * deactivate_user
	 * Deactivates a users company, preventing them from logging in.
	 *
	 * @return void
	 * @author Rob Hussey
	 * @author Mathew Davies
	 ...
	 * Verifies a submitted $update_email_token and updates their company with the new email address.
	 *
	 * @return bool
	 * @author Rob Hussey
	 */
	...
	public function get_unactivated_users($inactive_days = 28, $sql_select = FALSE, $sql_where = FALSE)
	{
		...
		// Do not delete companies added within set $inactive_days.
	...
	public function login($identity = FALSE, $password = FALSE, $remember_user = FALSE)
	{
		...
		if ($query->num_rows() == 1)
	    {
			...
			// If an activation time limit is defined by config file and company hasn't been activated by email.
			...
			// Check whether company has been activated.
			...
			// Check if company has been suspended.
	...
	/**
	 * update_last_login
	 * Updates the main user company table with the last time a user logged in and their IP address.
	...
	private function insert_database_login_session($user_id)
	{
	    ...
	    if ($this->db->affected_rows() > 0)
	    {
			...
			//a%3A4%3A%7Bs%3A10%3A%22session_id%22%3Bs%3A32%3A%229c3cac31ff09b2cd08656c36e9443cc2%22%3Bs%3A10%3A%22ip_address%22%3Bs%3A9%3A%22127.0.0.1%22%3Bs%3A10%3A%22user_agent%22%3Bs%3A112%3A%22Mozilla%2F5.0+%28Windows+NT+6.1%3B+Win64%3B+x64%29+AppleWebKit%2F537.36+%28KHTML%2C+like+Gecko%29+Chrome%2F38.0.2114.2+Safari%2F537.36%22%3Bs%3A13%3A%22last_activity%22%3Bi%3A1407471598%3B%7Df891a653bc1c96e8313b2bd24461e7223ce7acd7
	 ...
	 * Validate whether a non-activatated company is within the activation time limit set via config.
	 *
	 * @return bool
	 * @author Rob Hussey
	 */
	private function validate_activation_time_limit($last_login)
	{
		...
		// Set company activation expiry time.
		...
		// Company activation time has expired, user must now activate company via email.
	...
	public function send_email($email_to = NULL, $email_title = NULL, $data = NULL, $template = NULL)
	{
		...
		$this->email->from('support@tealcrm.com');
		$this->email->to($email_to);
		$this->email->subject("TealCRM ". $email_title);
...
*/
		...
		set_cookie($this->config->item('sess_cookie_name'));
	}
}
