...
class Flexi_auth_lite_model extends CI_Model
{
	public function __construct()
	{
		...
		// set db prefix from datamapper config
		get_dbprefix();
	...
	{
	    // Left Join user group table to user company table.
	    $this->db->join(
	...
	public function logout($all_sessions = TRUE)
	{
		...
		// kill session
		//$this->session->unset_userdata("teal_crm_defaults");
	...
	public function validate_database_login_session()
	{
		...
		// The users login session token has either expired, is invalid (Not found in database), or their company has been deactivated since login.
