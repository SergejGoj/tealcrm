...
*/
	...
	*/
	...
	/**
	 * Primary User Company Table
	 * The primary user company table contains all of the columns required for different functions within the flexi auth library.
	 *
	...
	*/
	...
	/**
	 * Additional custom tables that are directly related to the user company table can be included in flexi auth CRUD functions by
	 * setting their database structure via the the $config['database'] array.
	 *
	 * Typically, such examples of a custom table you may wish to link to the user company table would be a user profile table listing the
	...
	*/
	...
	$config['database']['custom']['user_profile']['custom_columns'] = array(
		'upro_first_name','upro_last_name','upro_phone','upro_newsletter','upro_filename_mimetype','upro_filename_original','language'
	);
	...
	*/
	 ...
	 *
	 * Note: The column MUST be either the ['email'] or ['username'] columns from the main user company table, and must contain a unique column name.
	*/
	...
	*/
	 ...
	 *
	 * Note: Any column within the user main company, custom or group tables can be added to array
	*/
	...
	*/
	$config['database']['settings']['date_time'] = gmdate('Y-m-d H:i:s');
	...
	*/
	 ...
	 *
	 * !IMPORTANT:
	 *	Do NOT change this salt once users have started registering companies as their passwords will not work without the original salt.
	 *	CHANGE THE DEFAULT STATIC SALT SET BELOW TO YOUR OWN RANDOM SET OF CHARACTERS.
	*/
	$config['security']['static_salt'] = 'Wauo89#6ffgDsr^aukNg88js6%a4d';
	...
	*/
	$config['security']['recaptcha_public_key']	= '6Lc3DvYSAAAAAEENMXqdbMyFsLj_EMqQZHpkjd6q';
	$config['security']['recaptcha_private_key'] = '6Lc3DvYSAAAAAEuvy5X-2if7JiCYIp8EMe3aeoU1';
	...
	*/
	...
	/**
	 * Set whether companies are suspended by default on registration / inserting user.
	 * This option allows admins to verify company details before enabling users.
	 * @param: bool
	*/
	$config['settings']['suspend_new_accounts'] = FALSE;

	/**
	 * Set a time limit to grant users instant login access, once expired, they are locked out until they activate their company via an activation email sent to them.
	 * @param: int
	 *
	...
	*/
	 ...
	 * Some of the functions in flexi auth need to send emails to the user (i.e. 'Company Activation', 'Forgot Password' etc).
	...
	*/
	...
	$config['email']['site_title'] = "Eligeo CRM";

	// Reply email shown as 'from' header on emails.
	$config['email']['reply_email'] = "hello@eligeo.com";
	...
	*/
	...
	/**
	 * 'Activate Company' email template.
